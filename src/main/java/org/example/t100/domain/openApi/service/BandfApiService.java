...
public class BandfApiService {
    ...
    @Autowired
    private BandfApiRepository bandfApiRepository;

    public String create_Prompt() {
        // 특정 범위의 트렌드 데이터를 한 번에 조회
        Pageable pageable = PageRequest.of(0, 25, Sort.by("id").descending());
        List<BandfTrend> trends = bandfTrendRepository.findAllByOrderByIdDesc(pageable);
...
//        List<Trend> trends = trendRepository.findByIdBetween(1L, 100L);
    ...
    public SuccessCode create_Report() {
        ...
        BandfOpenApi openApi = new BandfOpenApi();
        ...
        bandfApiRepository.save(openApi);

        return REPORT_CREATE_SUCCESS;
    }

    public BandfReportDto get_Report() {
        BandfOpenApi openApi = bandfApiRepository.findFirstByOrderByIdDesc();
        BandfReportDto reportDto= new BandfReportDto(openApi);
        return reportDto;
    }
