...
import org.example.t100.domain.crawler.repository.LandgTrendRepository;
...
import org.example.t100.domain.openApi.Dto.LandgReportDto;
...
import org.example.t100.domain.openApi.repository.LandgApiRepository;
...
public class LandgApiService {
    ...
    @Autowired
    private LandgTrendRepository landgTrendRepository;

    @Autowired
    private LandgApiRepository landgApiRepository;

    public String create_Prompt() {
        // 특정 범위의 트렌드 데이터를 한 번에 조회
        Pageable pageable = PageRequest.of(0, 25, Sort.by("id").descending());
        List<LandgTrend> trends = landgTrendRepository.findAllByOrderByIdDesc(pageable);
...
//        List<Trend> trends = trendRepository.findByIdBetween(1L, 100L);
    ...
    public SuccessCode create_Report() {
        ...
        LandgOpenApi openApi = new LandgOpenApi();
        ...
        landgApiRepository.save(openApi);

        return REPORT_CREATE_SUCCESS;
    }

    public LandgReportDto get_Report() {
        LandgOpenApi openApi = landgApiRepository.findFirstByOrderByIdDesc();
        LandgReportDto reportDto= new LandgReportDto(openApi);
        return reportDto;
    }
